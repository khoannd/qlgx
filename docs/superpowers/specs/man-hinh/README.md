...
| Giáo lý (Khối/Lớp/Học viên/Giáo lý viên) | `Giaoly/frmKhoiGiaoLyList.cs` + `frmKhoiGiaoLy.cs` + `frmLopGiaoLy.cs` + `frmHocSinh.cs` + `frmChuyenLop.cs` + `frmImportHocVien.cs` | 215+313+724+177+203+225 | xong (`giao-ly.md`) | xong — quản lý khối/lớp/học viên/giáo lý viên, "Chuyển lớp" và "Nhập học viên hàng loạt từ Excel" đều đã migrate (2026-09-08), xem `giao-ly.md` mục 8 |
