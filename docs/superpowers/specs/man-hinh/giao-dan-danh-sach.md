...
| Lọc theo giáo họ (kể cả giáo xóm con) | Một phần | Danh mục Giáo họ nay THẬT (`GET /api/giao-ho`, thay `data/giaoHoTam.ts` đã xoá — xem can-review-sau.md mục 19), nhưng `GiaoDanList.tsx` vẫn lọc theo `tenGiaoHo` so khớp chuỗi ở client (chấp nhận được ở quy mô 2050 giáo dân); backend `LayDanhSach` hỗ trợ `giaoHoId` nhưng không lọc theo `MaGiaoHoCha`/`GiaoHoChaId` (giáo xóm con) như desktop |
