...
/// <summary>
/// Chuỗi kết nối dùng cho NĂM đường dẫn hợp lệ truy vấn CHÉO GIÁO XỨ trong toàn hệ thống — đăng
/// nhập (AuthService, tra tên tài khoản trên toàn máy chủ trước khi biết giáo xứ), tạo tài
/// khoản quản trị đầu tiên (TaoTaiKhoanQuanTri), công cụ chuyển dữ liệu dòng lệnh (Qlgx.Migration,
/// dùng chuỗi kết nối riêng truyền qua tham số dòng lệnh, không đọc từ đây), màn hình "Quản lý
/// giáo phận/giáo hạt/giáo xứ" (QuanLyGiaoXuService) và màn hình "Nhập dữ liệu Access"
/// (NhapDuLieuService) — hai màn hình sau CHỈ gọi được qua policy "QuanTriHeThong", xem
...
/// truy vấn nghiệp vụ, mất hẳn lớp phòng thủ thứ hai. Các đường dẫn ở trên vẫn cần đọc/ghi chéo
