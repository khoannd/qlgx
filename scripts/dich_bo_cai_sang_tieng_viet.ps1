<#
====================================================================================
 Dịch giao diện bộ cài sang tiếng Việt
====================================================================================
 ...
 CÁI BẪY ĐÃ LÀM HỎNG MỘT LẦN - ĐỌC KỸ TRƯỚC KHI SỬA BẢNG DỊCH:
 Mỗi dòng trong bảng dịch phải bọc phần nối chuỗi trong ngoặc đơn:

     ,@('WelcomeForm','NextButton', ($F_THUONG + '&Tiếp >'))      ĐÚNG
     ,@('WelcomeForm','NextButton',  $F_THUONG + '&Tiếp >' )      SAI

 Không có ngoặc đơn thì PowerShell tách $F_THUONG và chuỗi thành HAI phần tử
 riêng, dòng đó thành 4 phần tử, và phần tử thứ ba chỉ còn lại thẻ phông rỗng.
 Kết quả: bộ cài ghi chuỗi trống vào mọi nút, toàn bộ màn hình cài đặt trắng
 trơn mà không có lỗi nào báo ra. Đúng chuyện đã xảy ra ở bản 4.0.1.

 Bên dưới có bước tự kiểm tra cấu trúc bảng dịch để chặn đúng cái bẫy đó.
...
# ==================================================== Tu kiem tra bang dich
# Chay TRUOC moi thao tac, trong ca hai che do. Bat dung cai bay mo ta o dau file:
# neu quen boc ngoac don thi dong do co 4 phan tu thay vi 3, va phan chu chi con
# lai the phong rong.
$loiBang = @()
foreach ($d in $dsControl) {
    if ($d.Count -ne 3) { $loiBang += "dong Control $($d[0]).$($d[1]) co $($d.Count) phan tu thay vi 3 - thieu ngoac don" }
    elseif ([string]$d[2] -match '^\{\[^}]*\}$') { $loiBang += "dong Control $($d[0]).$($d[1]) khong co chu, chi co the phong" }
}
foreach ($d in $dsRadio) {
    if ($d.Count -ne 3) { $loiBang += "dong RadioButton $($d[0]) #$($d[1]) co $($d.Count) phan tu thay vi 3 - thieu ngoac don" }
    elseif ([string]$d[2] -match '^\{\[^}]*\}$') { $loiBang += "dong RadioButton $($d[0]) #$($d[1]) khong co chu" }
}
foreach ($d in $dsTieuDe) {
    if ($d.Count -ne 2) { $loiBang += "dong tieu de $($d[0]) co $($d.Count) phan tu thay vi 2" }
}
# Phai co nhieu dong mang chu tieng Viet co dau, neu khong la file script bi doc
# sai bang ma (PowerShell doc file .ps1 khong co BOM theo bang ma ANSI).
$soCoDau = @($dsControl | Where-Object { @(([string]$_[2]).ToCharArray() | Where-Object { [int]$_ -gt 127 }).Count -gt 0 }).Count
if ($soCoDau -lt 40) { $loiBang += "chi co $soCoDau dong co chu tieng Viet co dau - script co the bi doc sai bang ma" }

if ($loiBang.Count -gt 0) {
    Write-Host 'LOI trong bang dich:'
    foreach ($l in $loiBang) { Write-Host "  - $l" }
    exit 1
}
...
if ($ChiKiemChung) {
    ...
    # Doi hoi chu doc ra tu MSI phai CON DAU tieng Viet. Neu chi so sanh "doc ra co
    # bang chuoi mong doi khong" thi khi ca hai cung rong van bao dat - dung cai bay
    # da giup ban 4.0.1 hong ma van qua duoc vong kiem chung.
    $soDauTrongMsi = 0
    foreach ($d in $dsControl) {
        $t = DocMotO $db "SELECT ``Text`` FROM ``Control`` WHERE ``Dialog_``='$($d[0])' AND ``Control``='$($d[1])'"
        $soDauTrongMsi += @(([string]$t).ToCharArray() | Where-Object { [int]$_ -gt 127 }).Count
    }
    if ($soDauTrongMsi -lt 100) { $thieu += "chu trong MSI chi co $soDauTrongMsi ky tu co dau - dau tieng Viet da bi mat" }
...
Write-Host "Da dich $($dsControl.Count) muc chu, $($dsRadio.Count) o chon, $($dsTieuDe.Count) tieu de; doi phong sang $PHONG"
